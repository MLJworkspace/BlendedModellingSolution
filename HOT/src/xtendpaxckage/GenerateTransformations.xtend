...
// load the resources
def doEMFSetup(String pathname){ 
 ...
 inject = new MappingStandaloneSetup().createInjectorAndDoEMFRegistration()
	...
	ecoreResourceSource.load(null);
	ecoreResourceTarget.load(null);
	resource.load(null);	
...
def transformEnum(MappingRule childRule){
	print ("    result." + (childRule.target as EAttribute).name)
	generateAssignment(childRule)
	print ((childRule.source as EEnum).name + " :: " + childRule.EEnumLiteral + ";")
    println
	if (childRule.condition !== null){
			print(" -> " + childRule.condition)
	}
...
def void findPath  (EObject source, EReference targetReference, MappingRule childRule){
   ...
   if ((source as EClass).EReferences.contains(targetReference) || 
   	targetReferenceOfSuperClass(source, targetReference) || 
   	elementOfStackAttributePathContainstargetReference(stackAttributePath, targetReference) === true){
...
//////////////////////////////////////////////////////////////////////////////////////////////
	def static void main(String[] args) {
       ...
       nexti = sc.next
		...
		new GenerateTransformations().doEMFSetup(inputMappingModel);
		
///Users/mlj01/runtime-Transf_Generator_HOTs/HOT/Graphical2Textual.mapping
	///Users/mlj01/git/TransformationGeneration/GeneratedTransformations/transforms/testGraphical2Textual.qvto
	///Users/mlj01/runtime-Transf_Generator_HOTs/GenTransformations/transforms/testGraphical2Textual.qvto
	}
}
